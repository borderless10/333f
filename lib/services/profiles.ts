...
export async function buscarUsuariosComPerfis() {
  try {
    ...
    if (error) {
      console.error(
        "[buscarUsuariosComPerfis] Erro ao buscar usuários:",
        error,
      );
      return { data: null, error };
    }
    ...
    console.log(
      "[buscarUsuariosComPerfis] Usuários encontrados:",
      data?.length || 0,
    );
...
export async function criarPerfil(userId: string, role: UserRole) {
  ...
  if (!role || !["admin", "analista", "viewer"].includes(role)) {
    throw new Error(
      `Role inválido: ${role}. Deve ser 'admin', 'analista' ou 'viewer'`,
    );
  }

  console.log(
    `[criarPerfil] Criando perfil para userId: ${userId}, role: ${role}`,
  );
...
export async function deletarUsuarioPermanentemente(userId: string) {
  ...
  const { data: rpcData, error: rpcError } = await supabase.rpc(
    "deletar_usuario_permanentemente",
    {
      p_user_id: userId,
    },
  );
...
async function confirmarEmailUsuario(userId: string): Promise<boolean> {
  try {
    ...
    if (error) {
      ...
      console.warn(
        "[confirmarEmailUsuario] Não foi possível confirmar email automaticamente:",
        error.message,
      );
      return false;
    }
    ...
    // Loga o erro mas não interrompe o fluxo
    console.warn(
      "[confirmarEmailUsuario] Erro ao confirmar email:",
      err?.message || err,
    );
    return false;
  }
...
export async function criarNovoUsuario(
  ...
  console.log(
    `[criarNovoUsuario] Iniciando criação de usuário com role: ${role}`,
  );
  ...
  if (!rpcError && rpcData) {
    console.log(
      "[criarNovoUsuario] RPC retornou sucesso, verificando perfil...",
    );
    const userIdFromRpc = rpcData.user_id || rpcData.id || rpcData.user?.id;

    if (!userIdFromRpc) {
      console.warn(
        "[criarNovoUsuario] RPC retornou sucesso mas sem user_id, continuando com fallback...",
      );
      // Se a RPC não retornou o ID do usuário, continua com o método fallback
    } else {
      // Verifica se o perfil foi criado pela RPC
      const { data: perfilExistente, error: perfilError } =
        await buscarPerfilUsuario(userIdFromRpc);

      if (perfilExistente) {
        console.log(
          "[criarNovoUsuario] Perfil já existe (criado pela RPC):",
          perfilExistente,
        );
        // Verifica se o role está correto
        if (perfilExistente.role !== role) {
          console.warn(
            `[criarNovoUsuario] Role do perfil (${perfilExistente.role}) não corresponde ao esperado (${role}), atualizando...`,
          );
          try {
            ...
            console.error(
              "[criarNovoUsuario] Erro ao atualizar role:",
              updateError,
            );
          }
        }
        ...
        console.log(
          "[criarNovoUsuario] RPC criou usuário mas não criou perfil, criando perfil manualmente...",
        );
        try {
          const perfil = await criarPerfil(userIdFromRpc, role);
          console.log(
            "[criarNovoUsuario] Perfil criado manualmente com sucesso:",
            perfil,
          );
          ...
          setCreatingUserFlag(false);
          console.error(
            "[criarNovoUsuario] Erro ao criar perfil após RPC:",
            perfilError,
          );
          throw new Error(
            "Usuário criado pela RPC, mas não foi possível criar o perfil: " +
              perfilError.message,
          );
        }
      }
  ...
  // Usa variáveis de ambiente definidas no .env / ambiente de deploy
  ...
  try {
    ...
    if (adminSession) {
      ...
      try {
        for (let i = 0; i < 3; i++) {
          ...
          if (restoreError) {
            console.warn(
              `Tentativa ${i + 1} de restaurar sessão falhou:`,
              restoreError,
            );
          }
        ...
        if (!sessaoRestaurada) {
          ...
          if (finalError) {
            console.error(
              "Erro na tentativa final de restaurar sessão:",
              finalError,
            );
          }
        }
    ...
    try {
      console.log(
        `Criando perfil para usuário ${novoUserId} com role: ${role}`,
      );
    ...
    if (adminSession) {
      ...
      if (finalVerifySession?.session?.user?.id !== adminUserId) {
        ...
        console.warn(
          "Sessão do admin foi alterada após confirmar email, restaurando...",
        );
        await supabase.auth.setSession({
          access_token: adminSession.access_token,
          refresh_token: adminSession.refresh_token,
    ...
    if (adminSession) {
      try {
        ...
        if (verifySession?.session?.user?.id !== adminUserId) {
          console.warn(
            "[criarNovoUsuario] Restaurando sessão do admin após erro...",
          );
          await supabase.auth.setSession({
            access_token: adminSession.access_token,
            refresh_token: adminSession.refresh_token,
        ...
        console.error(
          "Erro ao restaurar sessão do admin após erro:",
          restoreError,
        );
      }
    }
