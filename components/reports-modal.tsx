...
export function ReportsModal({ visible, onClose }: ReportsModalProps) {
  ...
  const [reconciliationData, setReconciliationData] =
    useState<ReconciliationReportData | null>(null);
  const [cashFlowData, setCashFlowData] = useState<CashFlowReportData | null>(
    null,
  );
    ...
    if (csvContent) {
      try {
        const fileUri =
          ...
          `relatorio_${selectedReport}_${Date.now()}.csv`;
  ...
  const handleExportPDF = async () => {
    if (!selectedReport) return;
    const data =
      selectedReport === "reconciliation" ? reconciliationData : cashFlowData;
    if (!data) return;
                  ...
                  <View style={styles.cardHeader}>
                    ...
                    <View style={styles.cardInfo}>
                      ...
                      <ThemedText style={styles.cardDescription}>
                        Extrato conciliado x não conciliado, com dados reais e
                        exportação CSV
                      </ThemedText>
                    </View>
                  </View>
                ...
                <GlassContainer style={styles.cardContent}>
                  <View style={styles.cardHeader}>
                    ...
                    <View style={styles.cardInfo}>
                      ...
                      <ThemedText style={styles.cardDescription}>
                        Análise de entradas, saídas e saldo por período (diário,
                        semanal, mensal)
                      </ThemedText>
                    </View>
                  </View>
                ...
                <View style={styles.filterRow}>
                  ...
                  >
                    <Text style={styles.accountPickerText}>
                      {selectedAccount
                        ? contas.find((c) => c.id === selectedAccount)
                            ?.descricao
                        : "Todas as contas"}
                    </Text>
                    <IconSymbol
                  ...
                  {showAccountPicker && (
                    <View style={styles.accountPickerDropdown}>
                      <TouchableOpacity
                        ...
                        activeOpacity={0.7}
                      >
                        <ThemedText style={styles.accountPickerOptionText}>
                          Todas as contas
                        </ThemedText>
                      </TouchableOpacity>
                      {contas.map((c) => (
                  ...
                  >
                    <GlassContainer style={styles.summaryCard}>
                      ...
                      <ThemedText style={styles.periodText}>
                        Período:{" "}
                        {formatReportDate(reconciliationData.period.start)} a{" "}
                        {formatReportDate(reconciliationData.period.end)}
                      </ThemedText>
                      ...
                      <View style={styles.summaryGrid}>
                        ...
                        >
                          ...
                          <ThemedText style={styles.summaryMeta}>
                            {reconciliationData.transacoesConciliadas} trans. ·{" "}
                            {reconciliationData.titulosConciliados} tít.
                          </ThemedText>
                        </View>
                        <View style={styles.summaryItem}>
                          ...
                          >
                            {formatCurrency(
                              reconciliationData.totalNaoConciliado,
                            )}
                          </Text>
                          <ThemedText style={styles.summaryMeta}>
                            Sobras:{" "}
                            {formatCurrency(
                              reconciliationData.totalSobrasValor,
                            )}{" "}
                            · Faltas:{" "}
                            {formatCurrency(
                              reconciliationData.totalFaltasValor,
                            )}
                          </ThemedText>
                        </View>
                        <View style={styles.summaryItem}>
                      ...
                      {reconciliationData.conciliados.length > 0 && (
                        <View style={styles.reportSection}>
                          <View style={styles.reportSectionHeader}>
                            ...
                            <ThemedText
                              type="defaultSemiBold"
                              style={styles.reportSectionTitle}
                            >
                              Conciliados (
                              {reconciliationData.conciliados.length})
                            </ThemedText>
                          </View>
                          <View style={styles.reportList}>
                            {reconciliationData.conciliados
                              .slice(0, 15)
                              .map((c) => (
                                <View
                                  key={`${c.transacao_id}-${c.titulo_id}`}
                                  style={styles.reportRow}
                                >
                                  <View style={styles.reportRowContent}>
                                    <ThemedText
                                      style={styles.reportRowDesc}
                                      numberOfLines={1}
                                    >
                                      {c.descricao_tx} ↔ {c.descricao_titulo}
                                    </ThemedText>
                                    <ThemedText style={styles.reportRowMeta}>
                                      {formatReportDate(c.data_transacao)} ·{" "}
                            ...
                            {reconciliationData.conciliados.length > 15 && (
                              <ThemedText style={styles.reportMore}>
                                +{reconciliationData.conciliados.length - 15} no
                                CSV
                              </ThemedText>
                            )}
                          </View>
                      ...
                      <View style={styles.summaryItem}>
                        ...
                        <Text
                          ...
                          style={[
                            ...
                            {
                              color:
                                cashFlowData.totals.netBalance >= 0
                                  ? "#10B981"
                                  : "#EF4444",
                            },
                          ]}
