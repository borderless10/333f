...
import { SobrasFaltasSection } from "./sobras-faltas-section";
...
export function ReconciliationModal({
  ...
  const [matchWasRunWithZeroSuggestions, setMatchWasRunWithZeroSuggestions] =
    useState(false);
  const [listViewMode, setListViewMode] = useState<"sobras" | "faltas" | null>(
    null,
  );
  const [sobrasFaltasActiveTab, setSobrasFaltasActiveTab] = useState<
    "sobras" | "faltas" | null
  >(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [txData, titlesData, contasData] = await Promise.all([
        ...
        buscarContas(userId, selectedCompany?.id ?? null).catch((err) => {
      ...
      if (filterAccount) {
        filteredTx = filteredTx.filter(
          (t) => t.conta_bancaria_id === filterAccount,
        );
  ...
  const generateAutoMatches = async () => {
    ...
    try {
      setMatching(true);
      const matches = await generateMatchSuggestions(
        userId,
        undefined,
        filterAccount,
      );
        ...
        >
          ...
          <View style={styles.modalHeader}>
            {listViewMode ? (
              <>
                ...
                <ThemedText
                  type="title"
                  style={[styles.modalTitle, styles.modalTitleWithBack]}
                >
          ...
          {!listViewMode && (
            ...
            >
                ...
                <GlassContainer style={styles.filterContainer}>
                  ...
                  >
                    ...
                    <Text style={styles.filterText}>
                      {filterAccount
                        ? contas.find((c) => c.id === filterAccount)
                            ?.descricao || "Todas"
                        : "Todas as contas"}
                    </Text>
                  </TouchableOpacity>
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      matching && styles.matchButtonTextDisabled,
                    ]}
                  >
              ...
              {!loading &&
                !matching &&
                matchWasRunWithZeroSuggestions &&
                  ...
                  <View style={styles.noSuggestionsBanner}>
                    ...
                    <View style={styles.noSuggestionsTextWrap}>
                      <ThemedText
                        ...
                        style={styles.noSuggestionsTitle}
                      >
                        Nenhuma sugestão encontrada
                      </ThemedText>
                      <ThemedText style={styles.noSuggestionsMessage}>
                        Tente conciliar manualmente: selecione uma transação na
                        coluna da esquerda (Banco) e um título na coluna da
                        direita (ERP), depois toque em Conciliar.
                      </ThemedText>
                    </View>
                  </View>
                )}
                    ...
                    >
                      {transactions.length === 0 ? (
                        <View style={styles.emptyColumn}>
                          ...
                          />
                          <ThemedText style={styles.emptyColumnText}>
                            Tudo conciliado!
                          </ThemedText>
                        </View>
                      ) : (
                                ...
                                <View style={styles.itemBottomSection}>
                                  <View style={styles.itemMetaRow}>
                                    ...
                                    <View style={styles.pendenteBadge}>
                                      <Text
                                        style={styles.pendenteBadgeText}
                                        numberOfLines={1}
                                      >
                                        Pendente
                                      </Text>
              ...
              {selectedTransaction && selectedTitle && (
                <View style={styles.reconcileButtonContainer}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      handleReconcile(selectedTransaction, selectedTitle)
                    }
                    activeOpacity={0.8}
                  >
                    <IconSymbol
