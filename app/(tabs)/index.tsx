...
import {
  ...
  shareReconciliationReport,
} from "@/lib/services/reconciliation-export";
import {
  generateReconciliationReport as getReconciliationReportData,
...
export default function DashboardScreen() {
  ...
  const [newTransactionVisible, setNewTransactionVisible] =
    React.useState(false);
  const [reconciliationVisible, setReconciliationVisible] =
    React.useState(false);
  const [reconciliationHistoryVisible, setReconciliationHistoryVisible] =
    React.useState(false);
  ...
  const [reconciliationSummary, setReconciliationSummary] =
    React.useState<ReconciliationReportData | null>(null);
  const [reconciliationSummaryLoading, setReconciliationSummaryLoading] =
    React.useState(false);
  const [reconciliationSummaryError, setReconciliationSummaryError] =
    React.useState<string | null>(null);
  const insets = useSafeAreaInsets();
  const { userId } = useAuth();
  const { selectedCompany } = useCompany();
  const { showSuccess, showError, showTransactionSuccess } = useNotification();
  ...
  React.useEffect(() => {
    ...
    const safeResetValue = (anim: Animated.Value, initialValue: number) => {
      try {
        ...
        console.warn(
          "Erro ao resetar valor de animação (pode ser ignorado durante cleanup):",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    getReconciliationReportData(
      userId,
      periodDates.startStr,
      periodDates.endStr,
    )
      .then(({ data, error }) => {
        if (cancelled) return;
        if (error) {
          setReconciliationSummaryError(
            error?.message ?? "Erro ao carregar conciliação",
          );
          setReconciliationSummary(null);
          return;
        }
      ...
      .catch((err) => {
        if (!cancelled) {
          setReconciliationSummaryError(
            err?.message ?? "Erro ao carregar conciliação",
          );
          setReconciliationSummary(null);
        }
      })
  ...
  const financialData = useMemo(() => {
    const income = filteredTransactions
      ...
      .reduce(
        (sum: number, t: TransactionWithAccount) => sum + Number(t.valor),
        0,
      );
  ...
  const comparison = useMemo(() => {
    ...
    return {
      balance: calculateChange(
        ...
        previousFinancialData.balance,
      ),
      income: calculateChange(
        financialData.income,
        previousFinancialData.income,
      ),
      expense: calculateChange(
        financialData.expense,
        previousFinancialData.expense,
      ),
    };
  ...
  const alerts = useMemo(() => {
    ...
    if (titulosVencendo.length > 0) {
      const totalVencendo = titulosVencendo.reduce(
      ...
      >
        ...
        >
          ...
          <View style={styles.periodSelector}>
            {(["today", "week", "month", "year"] as PeriodType[]).map(
              (period) => {
            ...
            <GlassContainer style={styles.balanceCard}>
              ...
              {comparison.balance.percentage > 0 && (
                <View style={styles.balanceTrend}>
                  <IconSymbol
                    name={
                      comparison.balance.isPositive
                        ? "arrow.up.right"
                        : "arrow.down.right"
                    }
                    size={14}
                    color={
                      comparison.balance.isPositive ? "#10B981" : "#EF4444"
                    }
                  />
                  <Text
                    style={[
                      ...
                      {
                        color: comparison.balance.isPositive
                          ? "#10B981"
                          : "#EF4444",
                      },
                    ]}
              ...
              <GlassContainer style={styles.squareCard}>
                ...
                {comparison.income.percentage > 0 && (
                  <View style={styles.squareTrend}>
                    <IconSymbol
                      name={
                        comparison.income.isPositive
                          ? "arrow.up.right"
                          : "arrow.down.right"
                      }
                      size={12}
                      color={
                        comparison.income.isPositive ? "#10B981" : "#EF4444"
                      }
                    />
                    <Text
                      style={[
                        ...
                        {
                          color: comparison.income.isPositive
                            ? "#10B981"
                            : "#EF4444",
                        },
                      ]}
              ...
              <GlassContainer style={styles.squareCard}>
                ...
                {comparison.expense.percentage > 0 && (
                  <View style={styles.squareTrend}>
                    <IconSymbol
                      name={
                        comparison.expense.isPositive
                          ? "arrow.up.right"
                          : "arrow.down.right"
                      }
                      size={12}
                      color={
                        comparison.expense.isPositive ? "#10B981" : "#EF4444"
                      }
                    />
                    <Text
                      style={[
                        styles.squareTrendText,
                        {
                          color: comparison.expense.isPositive
                            ? "#10B981"
                            : "#EF4444",
                        },
                      ]}
                ...
                <GlassContainer style={styles.categoryCard}>
                  ...
                  {categorySummary.topExpenses.map((item, index) => (
                    <View key={item.categoria} style={styles.categoryItem}>
                      ...
                      <Text
                        style={[styles.categoryValue, { color: "#EF4444" }]}
                      >
                ...
                <GlassContainer style={styles.categoryCard}>
                  ...
                  {categorySummary.topIncomes.map((item, index) => (
                    <View key={item.categoria} style={styles.categoryItem}>
                      ...
                      <Text
                        style={[styles.categoryValue, { color: "#10B981" }]}
                      >
                        {formatCurrency(item.valor)}
                      </Text>
                    </View>
        ...
        >
          ...
          {reconciliationSummaryLoading && (
            <View style={styles.reconciliationStatsRow}>
              <GlassContainer style={styles.reconciliationStatCardSkeleton}>
                ...
                <ThemedText style={styles.reconciliationStatSkeletonText}>
                  Carregando...
                </ThemedText>
              </GlassContainer>
            </View>
          ...
          {!reconciliationSummaryLoading && reconciliationSummaryError && (
            <View style={styles.reconciliationStatsRow}>
              <GlassContainer style={styles.reconciliationStatCardError}>
                ...
                />
                <ThemedText
                  style={styles.reconciliationStatErrorText}
                  numberOfLines={2}
                >
          ...
          {!reconciliationSummaryLoading && reconciliationSummary && (
            <View style={styles.reconciliationStatsGrid}>
              <GlassContainer style={styles.reconciliationStatCard}>
                <View
                  style={[
                    styles.reconciliationStatIconWrap,
                    { backgroundColor: "#00b09b20" },
                  ]}
                >
                  <IconSymbol name="percent" size={20} color="#00b09b" />
                </View>
                <ThemedText style={styles.reconciliationStatLabel}>
                  Taxa
                </ThemedText>
          ...
          <GlassContainer style={styles.reconciliationCard}>
            <View style={styles.reconciliationContent}>
              ...
              <View style={styles.reconciliationTextContainer}>
                <ThemedText
                  type="defaultSemiBold"
                  style={styles.reconciliationTitle}
                >
            ...
            >
              ...
              <Text style={styles.reconciliationButtonText}>
                Iniciar Conciliação
              </Text>
            </TouchableOpacity>
          </GlassContainer>
          <View style={styles.reconciliationActions}>
            <Animated.View
              style={{ flex: 1, transform: [{ scale: scaleAnim5 }] }}
            >
              <TouchableOpacity
            ...
            <Animated.View
              style={{ flex: 1, transform: [{ scale: scaleAnim6 }] }}
            >
              <TouchableOpacity
                style={styles.reconciliationActionButton}
        ...
        >
          ...
          <View style={styles.quickActionsContainer}>
            <View style={styles.quickActions}>
              <Animated.View
                style={{ flex: 1, transform: [{ scale: scaleAnim1 }] }}
              >
                <TouchableOpacity
              ...
              <Animated.View
                style={{ flex: 1, transform: [{ scale: scaleAnim2 }] }}
              >
                <TouchableOpacity
            ...
            <View style={styles.quickActions}>
              <Animated.View
                style={{ flex: 1, transform: [{ scale: scaleAnim3 }] }}
              >
                <TouchableOpacity
              ...
              <Animated.View
                style={{ flex: 1, transform: [{ scale: scaleAnim4 }] }}
              >
                <TouchableOpacity
